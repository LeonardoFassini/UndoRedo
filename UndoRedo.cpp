...
int main(void){
  ...
  carregarBanco();
  printf("\nDados do banco antes da recuperação:\n");
  printarBanco();
  analisarLog();
  undu();
  redu();
...
//                  COMMIT CASE
        ...
        tmpname[k] = '\0';
		if(!passouSc) //Agora confere tambem se nao passou do startcheckpoint
...
//                  OPERATION CASE
        ...
        if(aux == transacoes.end() && !passouSc){ //Agora confere tambem se nao passou do startcheckpoint
